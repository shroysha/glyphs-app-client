...
namespace unitycoder_MobilePaint
{
    ...
    public class MobilePaint : MonoBehaviour
    {
		...
		*/
		...
		public void addSnapshot(Texture2D snapshot) {

			float parentWidth = texWidth;
			float parentHeight = texHeight;

			float snapshotWidth = (float) snapshot.width;
			float snapshotHeight = (float) snapshot.height;

			// how many snapshot pixels represent 1 parent pixel
			float pixelRatio = Mathf.Min (parentWidth / snapshotWidth, parentHeight / snapshotHeight);
			 ...
			 * canvas is 400x300
			 * 
			 * snapshot top left = 0,0 on canvas = 0 byte position
			 * snapshot top right = 400,0 on canvas = 399 byte position
			 * snapshot bottom left = 0,300 on canvas
			 * snapshot bottom right = 400,300 on canvas
			 * 
			 * we can then convert those points to byte array positions
			 * 
			 * 
			 */
			// iterate through snapshot pixels and place them in canvas pixels
			for (int i = 0; i < snapshotPixels.Length; i ++) { 
				Color pixelColor = snapshotPixels [i];

				int currentSnapshotX = i % (int)snapshotWidth;
				int currentSnapshotY = i / (int)snapshotWidth;

				int transposedX = (int)((float)currentSnapshotX * pixelRatio);
				int transposedY = (int)((float)currentSnapshotY * pixelRatio);

				drawingTexture.SetPixel (transposedX, transposedY, pixelColor);

				/*
				int positionInTexByteArray = transposedX * (int)parentWidth + transposedY * (int)parentHeight * 4;
				Debug.Log ("Length " + pixels.Length + " Trying to get " + positionInTexByteArray);
				pixels[positionInTexByteArray] = pixelColor.r;
				pixels[positionInTexByteArray + 1] = pixelColor.g;
				pixels[positionInTexByteArray + 2] = pixelColor.b;
				pixels[positionInTexByteArray + 3] = pixelColor.a;*/
			}

			drawingTexture.Apply (false);
			pixels = drawingTexture.GetRawTextureData ();
			//textureNeedsUpdate = true;
		}
